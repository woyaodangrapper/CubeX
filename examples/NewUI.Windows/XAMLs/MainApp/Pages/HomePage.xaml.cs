...
using static XLauncher.Dialogs.SimpleDialogs;
...
# endif
        ...
        private async void InstallGameDialog(object sender, RoutedEventArgs e)
        {
            ...
            catch (NullReferenceException ex)
            {
                ...
                ErrorSender.SendException(new NullReferenceException("Collapse was not able to complete post-installation tasks, but your game has been successfully updated.\r\t" +
                    $"Please report this issue to our GitHub here: https://github.com/neon-nyan/XLauncher/issues/new or come back to the launcher and make sure to use Repair Game in Game Settings button later.\r\nThrow: {ex}", ex));
            }
            catch (Exception ex)
            {
                IsPageUnload = true;
        ...
        private static int ReadPlaytimeFromRegistry(string RegionRegKey)
        {
            try
            {
                return (int)Registry.CurrentUser.OpenSubKey(RegionRegKey, true).GetValue("XLauncher_Playtime", 0);
            }
            catch (Exception ex)
            {
        ...
        private static void SavePlaytimetoRegistry(string RegionRegKey, int value)
        {
            try
            {
                Registry.CurrentUser.OpenSubKey(RegionRegKey, true).SetValue("XLauncher_Playtime", value, RegistryValueKind.DWord);
            }
            catch (Exception ex)
            {
